...
export class ProcessAllVaultModal extends Modal {
    ...
    private createGeneralSettings(contentEl: HTMLElement) {
        new Setting(contentEl)
            .setName("转换为 ⓘ")
            .setDesc(
                "选择输出格式。「与原格式相同」会对当前格式应用压缩/调整大小"
            )
            .setTooltip(
                "与原格式相同：保留当前格式，同时应用压缩/调整大小"
            )
            .addDropdown((dropdown) => {
                dropdown
                    .addOption("disabled", "与原格式相同")
                    .addOptions({
                        webp: "WebP",
                        jpg: "JPG",
                        png: "PNG",
    ...
    private createSkipSettings(contentEl: HTMLElement) {
        new Setting(contentEl)
            ...
            .setDesc(
                ...
                "以逗号分隔的列表（不含点或空格）。示例：png,gif"
            )
            .setTooltip(
                "以逗号分隔的要跳过的文件格式列表（例如 tif,tiff,heic）。留空则处理所有格式。"
            )
            .addText((text) => {
                text.setPlaceholder(
        ...
        new Setting(contentEl)
            ...
            .setTooltip(
                "如果图片已是目标格式，此选项允许您跳过其压缩、转换和调整大小。其他格式的图片仍会被处理。"
            )
            .addToggle((toggle) => {
                toggle
                    .setValue(
    ...
    private createEnlargeReduceSettings(): void {
        ...
        this.enlargeReduceSettings = new Setting(this.enlargeReduceDiv)
            ...
            .setName("放大或缩小 ⓘ")
            .setDesc(
                "缩小并放大：调整所有图片。仅缩小：缩小较大的图片。仅放大：放大较小的图片"
            )
            .setTooltip(
