...
export class ProcessCurrentNote extends Modal {
    ...
    async onOpen(): Promise<void> {
		...
		headerContainer.createEl("h6", {
			text: `当前笔记中的所有图片：${this.activeFile.basename}.${this.activeFile.extension}`,
			cls: "modal-subtitle",
		});
		...
		headerContainer.createEl("p", {
			...
			text: "⚠️ 此操作将修改当前笔记中的所有图片——请确保您已备份。",
		});
		...
		this.skipFormatsSetting = new Setting(skipContainer)
			.setName("跳过文件格式 ⓘ")
			.setTooltip(
				"以逗号分隔的要跳过的文件格式（例如 tif,tiff,heic）。留空则处理所有格式。",
			)
			.addText((text) =>
				text
		...
		behaviorList.createEl("li", {
			text: "不含通配符的文件夹路径会跳过该文件夹及所有子文件夹",
		});
		behaviorList.createEl("li", {
			text: "开头的 / 可省略",
		});
		...
		examplesList.createEl("li", {
			text: "_attachments → 跳过该文件夹及其中所有内容",
		});
		examplesList.createEl("li", {
			text: "/_attachments → 同上",
		});
		examplesList.createEl("li", {
			text: "_attachments/* → 仅跳过该文件夹中的直接子项",
		});
		examplesList.createEl("li", {
		...
		advancedExamplesList.createEl("li", {
			text: "/^_attachments\\// → 跳过以 _attachments/ 开头的路径",
		});
		advancedExamplesList.createEl("li", {
			text: "regex:^media/(raw|temp)/ → 跳过 media/raw 和 media/temp",
		});
		advancedExamplesList.createEl("li", {
			text: "r/^(drafts|scratch)\\// → 跳过 drafts/ 和 scratch/",
		});
		...
		this.skipTargetFormatSetting = new Setting(skipContainer)
			.setName("跳过已是目标格式的图片 ⓘ")
			.setTooltip(
				"如果图片已经是目标格式，则跳过其压缩、转换和调整大小。其他格式的图片仍会被处理。",
			)
			.addToggle((toggle) =>
				toggle
					.setValue(
    ...
    private createEnlargeReduceSettings(): void {
        ...
        this.enlargeReduceSettings = new Setting(this.enlargeReduceDiv)
            ...
            .setTooltip('• 缩小和放大：调整所有图片以适应指定尺寸\n• 仅缩小：只缩小大于目标的图片\n• 仅放大：只放大小于目标的图片')
            .addDropdown((dropdown) => {
                dropdown
                    .addOptions({
                        Always: '缩小和放大',
