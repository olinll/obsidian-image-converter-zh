...
export class ProcessFolderModal extends Modal {
    ...
    private createHeader(contentEl: HTMLElement) {
        ...
        // Main title
        headerContainer.createEl("h2", { text: "转换、压缩和调整大小" });

        // Subtitle
        headerContainer.createEl("h6", {
            text: `所有图片位于：/${folderName}`,
    ...
    private createWarningMessage(contentEl: HTMLElement) {
        contentEl.createEl("p", {
            ...
            text: "⚠️ 这将修改此文件夹及其子文件夹中的所有图片。请确保您已备份。",
        });
    }
    ...
    private createImageCountsDisplay(contentEl: HTMLElement) {
        ...
        // Image Counts
        countsDisplay.createEl("span", { text: "找到的图片总数：" });
        ...
        countsDisplay.createEl("span", { text: "将被跳过：" });
        ...
        countsDisplay.createEl("span", { text: "将被处理：" });
    ...
    private async createGeneralSettings(contentEl: HTMLElement) {
        ...
        this.convertToSetting = new Setting(contentEl)
            ...
            .setDesc(
                // eslint-disable-next-line obsidianmd/ui/sentence-case
                "选择输出格式。'与原图相同'将对当前格式应用压缩/调整大小。"
            )
            .setTooltip(
                "与原图相同：保留当前格式，同时应用压缩/调整大小"
            )
            .addDropdown((dropdown) => {
                dropdown
                    .addOption("disabled", "与原图相同")
                    .addOptions({
                        webp: "WebP",
                        jpg: "JPG",
                        png: "PNG",
    ...
    private createSkipSettings(contentEl: HTMLElement): void {
        contentEl.createEl("h4", { text: "跳过" }); // Heading for Skip Settings
        ...
        this.skipFormatsSetting = new Setting(contentEl)
            ...
            .setDesc(
                // eslint-disable-next-line obsidianmd/ui/sentence-case
                "逗号分隔列表（无点号或空格，例如 png,gif）。"
            )
            .setTooltip(
                "逗号分隔的要跳过的文件格式列表（例如 tif,tiff,heic）。留空则处理所有格式。"
            )
            .addText((text) => {
                text
                    // eslint-disable-next-line obsidianmd/ui/sentence-case
        ...
        this.skipTargetFormatSetting = new Setting(contentEl)
            .setName("跳过已是目标格式的图片 ⓘ")
            .setDesc(
                "如果图片已是目标格式，则跳过压缩/调整大小。"
            )
            .setTooltip(
                "如果图片已是目标格式，可以跳过其压缩、转换和调整大小。其他格式的处理仍将执行。"
            )
            .addToggle((toggle) => {
                toggle
                    .setValue(
    ...
    private async createResizeSettings(contentEl: HTMLElement) {
        contentEl.createEl("h4", { text: "调整大小" }); // Heading for Resize Settings
    ...
    private createEnlargeReduceSettings(): void {
        ...
        this.enlargeReduceSettings = new Setting(this.enlargeReduceDiv)
            ...
            .setDesc(
                // eslint-disable-next-line obsidianmd/ui/sentence-case
                "缩小和放大：调整所有图片。仅缩小：缩小较大的图片。仅放大：放大较小的图片。"
            )
            .setTooltip(
                // eslint-disable-next-line obsidianmd/ui/sentence-case
                "• 缩小和放大：调整所有图片以适应指定尺寸\n• 仅缩小：仅缩小大于目标的图片\n• 仅放大：仅放大小于目标的图片"
    ...
    private updateResizeInputSettings(resizeMode: string): void {
        ...
        if (["Fit", "Fill"].includes(resizeMode)) {
            ...
            switch (resizeMode) {
                case "LongestEdge":
                case "ShortestEdge":
                    name = resizeMode === "LongestEdge" ? "最长边" : "最短边";
                    desc = "输入所需的长度（像素）";
                    break;
                case "Width":
                    name = "宽度";
