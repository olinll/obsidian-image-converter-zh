...
export class ContextMenu extends Component {
	...
	private async handleDimensionsAndCaptionUpdate(
		...
		if (matches.length > 1) {
			new ConfirmDialog(
				this.app,
				"确认更新",
				`找到 ${matches.length} 个匹配的图片链接。是否全部更新？`,
				"更新",
				() => {
	...
	addRenameAndMoveInputs(
		...
		menu.addItem((item) => {
				...
				nameInput.placeholder = "输入新的图片名称";
				...
				pathInput.placeholder = "输入图片的新路径";
	...
	async cutImageAndLinkFromNote(event: MouseEvent) {
		...
		if (matches.length > 1) {
				...
				new ConfirmDialog(
					this.app,
					"确认剪切",
					`在当前笔记中找到 ${matches.length} 个匹配的图片链接。是否全部剪切？`,
					"剪切",
					() => {
						handleConfirmation().catch((error: unknown) => {
	...
	async copyImageAsBase64(event: MouseEvent) {
		...
		this.registerDomEvent(img, "load", async () => {
			try {
				...
				// eslint-disable-next-line obsidianmd/ui/sentence-case -- Base64 is a proper technical term
				new Notice("图片已作为 Base64 复制到剪贴板");
			} catch (error) {
				console.error("Failed to copy image as Base64:", error);
				// eslint-disable-next-line obsidianmd/ui/sentence-case -- Base64 is a proper technical term
				new Notice("复制图片为 Base64 失败");
			}
		});
	...
	async deleteImageAndLinkFromNote(event: MouseEvent) {
		...
		try {
			...
			if (uniqueMatches.length > 1) {
				...
				introText.textContent = `在当前笔记中找到 ${uniqueMatches.length} 个唯一匹配的图片链接。是否全部删除？`; // Updated message
				...
				uniqueMatches.forEach((match, index) => {
					...
					detailDiv.createSpan({ text: `  ${index + 1}. 第 ${lineNumber} 行：${lineContent}` });
